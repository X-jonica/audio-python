...
# === Clés API ===
AUDD_API_TOKEN_PUBLIC = '0fd99038f390e8b309cb872974d5ed91'
...
def analyze_audio_yamnet(file_path):
    try:
        waveform = librosa.load(file_path, sr=16000, mono=True)
...
def recognize_song(file_path, audd_token):
    """
    audd_token : peut être la clé publique ou la clé de l'utilisateur
    """
    try:
...
def recognize_from_upload():
    ...
    # Récupérer la clé AUDD de l'utilisateur si elle existe
    user = User.query.get(user_id)
    if user and hasattr(user, 'audd_key') and user.audd_key:
        audd_token = user.audd_key
    else:
